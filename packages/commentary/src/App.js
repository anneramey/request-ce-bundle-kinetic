...
import { NewILoIContainer } from './components/new_iloi/NewILoI';
import { LoIContainer } from './components/loi/letter';
...
const AppComponent = props => {
  if (props.error) {
    ...
    return props.render({
      ...
      main: (
        <I18n>
          <main className={`package-layout package-layout--commentary`}>
            <Router>
              <Settings path="/settings/*" homePath="../" />
              <Home path="/" />
              <Redirect from="/submissions/:id" to="/loi/:id" />
              <NewILoIContainer path="/new/iloi" />
              <LoIContainer path="/loi/:id" />
              <Form
...
const mapStateToProps = (state, props) => ({
  loading: state.commentaryApp.loading,
  error: state.commentaryApp.error,
});
